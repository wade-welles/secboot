...
// the specified branches. If the computed verification measurement has already been measured from the specified source in a branch,
// then it will not be measured again.
//
// In order to compute the measurement for each branch, the source of the certificate that will be used to verify the image needs to
// be determined. If the image is not signed with an authority that chains to a valid signature database for a particular branch,
// then that branch will be marked as unbootable and it will be omitted from the final PCR profile
...
// extends that to the specified branch. The branch is then updated to contain a list of signatures associated with the resulting
...
// extends that to the specified branch.
